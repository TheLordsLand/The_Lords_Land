    ...
    switch ($_GET['action'])
    {
      ...
      if (isset($module))
       switch ($module['type'])
       {
        case 'research':
         ...
         foreach ($game['technologies'][$node->data['faction']] as $key=>$item)
          if ($item['module']==$mid) $technologyList[]=$key;
         $node->getQueue('research', 'technology', $technologyList);
         $totalIR=0;
        ...
        case 'craft':
         $componentList=array();
         foreach ($game['components'][$node->data['faction']] as $key=>$item)
          if ($item['module']==$mid) $componentList[]=$key;
         $node->getQueue('craft', 'component', $componentList);
         $totalIR=0;
         foreach ($node->modules as $key=>$item)
          if ($item['module']==$mid) $totalIR+=$item['input']*$game['modules'][$node->data['faction']][$item['module']]['ratio'];
        break;
        case 'train':
         $unitList=array();
         foreach ($game['units'][$node->data['faction']] as $key=>$item)
          if ($item['module']==$mid) $unitList[]=$key;
         $node->getQueue('train', 'unit', $unitList);
         $totalIR=0;
         foreach ($node->modules as $key=>$item)
          if ($item['module']==$mid) $totalIR+=$item['input']*$game['modules'][$node->data['faction']][$item['module']]['ratio'];
        break;
     ...
     case 'addTechnology':
      if ($flags['research'])
      {
       ...
       {
        $status=$node->addTechnology($_GET['technologyId']);
        if ($status=='done') header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
        else $message=$ui[$status];
       }
      }
     ...
     case 'cancelTechnology':
      if (isset($_GET['technologyId']))
      {
       $status=$node->cancelTechnology($_GET['technologyId']);
       if ($status=='done') header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
       else $message=$ui[$status];
      }
     break;
     case 'addComponent':
      if ($flags['craft'])
      {
       if (isset($_GET['componentId'], $_POST['quantity']))
        if ($_POST['quantity']>0)
        {
         $status=$node->addComponent($_GET['componentId'], $_POST['quantity']);
         if ($status=='done') header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
         else $message=$ui[$status];
        }
     ...
     case 'removeComponent':
      if ($flags['craft'])
      {
       if (isset($_GET['componentId'], $_POST['quantity']))
       {
        $status=$node->removeComponent($_GET['componentId'], $_POST['quantity']);
        if ($status=='done') header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
        else $message=$ui[$status];
       }
      }
     ...
     case 'cancelComponent':
      ...
      {
       $status=$node->cancelComponent($_GET['craftId']);
       if ($status=='done') header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
       else $message=$ui[$status];
      }
     break;
     case 'addUnit':
      if ($flags['train'])
      {
       if (isset($_GET['unitId'], $_POST['quantity']))
        if ($_POST['quantity']>0)
        {
         $status=$node->addUnit($_GET['unitId'], $_POST['quantity']);
         if ($status=='done') header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
         else $message=$ui[$status];
        }
        else header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
     ...
     case 'removeUnit':
      if ($flags['train'])
      {
       if (isset($_GET['unitId'], $_POST['quantity']))
       {
        $status=$node->removeUnit($_GET['unitId'], $_POST['quantity']);
        if ($status=='done') header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
        else $message=$ui[$status];
       }
      }
     ...
     case 'cancelUnit':
      ...
      {
       $status=$node->cancelUnit($_GET['trainId']);
       if ($status=='done') header('Location: module.php?action=get&nodeId='.$node->data['id'].'&slotId='.$_GET['slotId']);
       else $message=$ui[$status];
      }
     break;
