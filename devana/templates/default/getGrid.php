...
for ($k = 3; $k >= -3; $k--)
{
   for ($j = -3; $j <= 3; $j++)
   {
      ...
      $st_y = (3 - $k) * 20 + ($j + 3) * 20;
?>
  <img style="position: absolute; left: <?php echo $st_x; ?>px; top: <?php echo $st_y; ?>px; width: 80px; height: 80px;" src="<?php echo $grid->getSectorImage($x+$j, $y+$k, $i); ?>">
<?php
   }
}
?>
...
?>
   <area shape="circle" coords="482,38,15" href="javascript: getContent('getGrid.php', '<?php echo 'x='.$x.'&y='.($y+3); ?>')" title="<?php echo $ui['North']; ?>">
   <area shape="circle" coords="77,241,15" href="javascript: getContent('getGrid.php', '<?php echo 'x='.$x.'&y='.($y-3); ?>')" title="<?php echo $ui['South']; ?>">
   <area shape="circle" coords="482,241,15" href="javascript: getContent('getGrid.php', '<?php echo 'x='.($x+3).'&y='.$y; ?>')" title="<?php echo $ui['East']; ?>">
   <area shape="circle" coords="77,38,15" href="javascript: getContent('getGrid.php', '<?php echo 'x='.($x-3).'&y='.$y; ?>')" title="<?php echo $ui['West']; ?>">
  </map>
 </div>
