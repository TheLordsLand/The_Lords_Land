...
<?php include 'templates/'.$_SESSION[$shortTitle.'User']['template'].'/header.php'; ?>
  <div class="container">
   <div class="cell"><div class="content" id="content" style="width: 600px; height: 370px; text-align: left;"></div></div>
<?php
$sc=count($grid->data);
$rc=sqrt($sc);
$minimap='';
for ($i=0; $i<$sc; $i++)
{
 ...
 $minimap.='<div class="sector" style="background-color: '.$sectorColor.';" id="sector_'.$grid->data[$i]['x'].'_'.$grid->data[$i]['y'].'" onClick="getContent(\'getGrid.php\', \'x='.$grid->data[$i]['x'].'&y='.$grid->data[$i]['y'].'\')"></div>';
 if (!(($i+1)%$rc)) $minimap.='<br>';
}
echo '<div class="cell"><div class="content" style="z-index: 10;">'.$minimap.'</div></div>';
?>
  </div>
<?php include 'templates/'.$_SESSION[$shortTitle.'User']['template'].'/footer.php'; ?>
 </body>
<script type="text/javascript">getContent("getGrid.php", "<?php if (isset($vars)) echo $vars; ?>")</script>
</html>
