  ...
  case "get":
   if (isset($msg->data['recipient']))
   {
    ...
    <div style="border-bottom: 1px solid black; padding-bottom: 5px; margin-bottom: 5px;">
     <div class="row"><div class="cell">'.$ui['sender'].'</div><div class="cell">'.$msg->data['senderName'].'</div></div>
     <div class="row"><div class="cell">'.$ui['subject'].'</div><div class="cell">'.$msg->data['subject'].'</div></div>
    </div>
  ...
  case "add":
   ...
   echo '
    <form method="post" action="?action=add">
     <div>
      <div class="row"><div class="cell">'.$ui['recipients'].'</div><div class="cell"><input class="textbox" type="text" name="recipients" value="'.$recipient.'" style="width: 200px;"></div></div>
      <div class="row"><div class="cell">'.$ui['subject'].'</div><div class="cell"><input class="textbox" type="text" name="subject" value="'.$subject.'" style="width: 200px;"></div></div>
     </div>
     <div class="row"><textarea class="textbox" name="body" style="width: 400px; height: 200px;">'.$body.'</textarea></div>
     <div class="row"><input class="button" type="submit" value="'.$ui['send'].'"></div>
    </form>';
  break;
  case "list":
   echo '<div><a class="link" href="?action=add">'.$ui['send'].'</a> | <a class="link" href="?action=removeAll">'.$ui['removeAll'].'</a></div><div style="border-top: 1px solid black; border-bottom: 1px solid black; padding: 5px 0 5px 0; margin: 5px 0 5px 0;">';
   foreach ($messages['messages'] as $message)
   {
    if (!$message->data['viewed']) $new=' style="text-decoration: underline;"';
    else $new='';
    $hours=round((time()-strtotime($message->data['sent']))/3600, 2);
    echo '<div class="row"><div class="cell"><a class="link" href="message.php?action=get&messageId='.$message->data['id'].'"'.$new.'>'.$message->data['subject'].'</a></div><div class="cell">'.$hours.' '.$ui['hours'].'</div><div class="cell"><a class="link" href="?action=remove&messageId='.$message->data['id'].'">x</a></div></div>';
   }
   echo '</div>';
   if ($pageCount>1)
   {
